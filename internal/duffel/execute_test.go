...
func TestExecuteEmptyTargetNoConflictingPackageItems(t *testing.T) {
	...
	logbuf := &bytes.Buffer{}
	err := Execute(req, false, os.Stderr, duftest.Logger(t.Name(), slog.LevelDebug, logbuf))
	if err != nil {
		t.Error(err)
	}
...
func TestExecuteDirErrors(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			logbuf := &bytes.Buffer{}
			err := Execute(r, false, os.Stderr, duftest.Logger(t.Name(), slog.LevelDebug, logbuf))
